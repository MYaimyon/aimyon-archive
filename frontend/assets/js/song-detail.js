...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!trackId) {
    setStatus("Track ID is required.", true);
    return;
  }
  ...
  async function fetchTrack(id) {
    setStatus("Loading track information...");
    try {
      ...
      setStatus("We couldn't load this track. Please try again later.", true);
    }
  }

  function render(track) {
    const titleJa = track.titleJa || "Untitled";
    ...
    if (track.album) {
      ...
      metaAlbumEl.innerHTML = `<a href="${albumLink}">${albumTitle}</a>`;
      infoAlbumEl.innerHTML = `<a href="${albumLink}">${albumTitle}</a>`;
    ...
    metaDurationEl.textContent = track.duration ? `Duration · ${track.duration}` : "";
  ...
  function renderStories(stories) {
    ...
    if (!stories.length) {
      storyPaneEl.innerHTML = '<p class="placeholder">No production stories yet.</p>';
      return;
    }
    ...
    storyPaneEl.innerHTML = stories.map(story => {
      ...
      if (story.sourceName) {
        ...
        if (story.sourceUrl) {
          ...
          metaParts.push(`<a href="${sourceUrl}" target="_blank" rel="noopener">${sourceName}</a>`);
        } else {
          metaParts.push(sourceName);
        }
      }
      if (story.language) {
        metaParts.push(`Language ${escapeHtml(story.language.toUpperCase())}`);
      }
