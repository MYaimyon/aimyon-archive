// News detail: load single item from API (fallback to latest when id missing)
document.addEventListener('DOMContentLoaded', () => {
  const qs = new URLSearchParams(location.search);
  const id = qs.get('id');
  const API_BASE = 'http://localhost:8080/api/news-events';
  ...
  const loadData = async () => {
    ...
    const res = await fetch(`${API_BASE}?size=1`);
  ...
  loadData()
    ...
    .catch(() => {
      el.content.innerHTML = '<p style="color:var(--text-secondary)">뉴스를 불러오지 못했어요.</p>';
    });
});
