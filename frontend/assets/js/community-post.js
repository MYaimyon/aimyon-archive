...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!postId) {
    status.error('Í≤åÏãúÍ∏Ä IDÍ∞Ä ?ÜÏäµ?àÎã§.');
    if (el.content) el.content.innerHTML = '<p>Í≤åÏãúÍ∏Ä??Ï∞æÏùÑ ???ÜÏäµ?àÎã§.</p>';
    return;
  }
  ...
  const updateCommentCounts = (count) => {
    ...
    if (totalLabel) totalLabel.innerHTML = `?ÑÏ≤¥ ?ìÍ? <strong>${n}</strong>Í∞?;
  };

  const updateLikeButton = () => {
    const count = currentPost?.likeCount ?? 0;
    const icon = isLiked ? '?? : '??;
    const label = isLiked ? 'Ï∂îÏ≤ú Ï∑®ÏÜå' : 'Ï∂îÏ≤ú';
  ...
  const renderPost = () => {
    ...
    if (el.title) el.title.textContent = title || '?úÎ™© ?ÜÏùå';
  ...
  const renderComments = () => {
    ...
    if (!currentComments.length) {
      el.cmtList.innerHTML = '<div class="community-empty"><p>?±Î°ù???ìÍ????ÜÏäµ?àÎã§</p></div>';
      return;
    }
    el.cmtList.innerHTML = currentComments.map((c) => {
      const author = c.author || `?åÏõê #${c.userId ?? '-'}`;
  ...
  const loadMockData = (fromFallback = false) => {
    ...
    if (!mp) { status.error('?òÌîå Í≤åÏãúÍ∏Ä??Ï∞æÏùÑ ???ÜÏäµ?àÎã§.'); return; }
    usingMock = true;
    currentPost = mp;
    currentComments = pickMockComments(mp.id);
    status.message(fromFallback ? 'Í≤åÏãúÍ∏Ä??Î∂àÎü¨?§Ï? Î™ªÌï¥ ?òÌîå ?∞Ïù¥?∞Î? Î≥¥Ïó¨Ï§çÎãà??' : '?òÌîå ?∞Ïù¥?∞Î? Î≥¥Ïó¨Ï£ºÎäî Ï§ëÏûÖ?àÎã§.');
    renderPost();
    renderComments();
  };
  ...
  const loadPost = () => {
    ...
    status.loading('Í≤åÏãúÍ∏Ä??Î∂àÎü¨?§Îäî Ï§ëÏûÖ?àÎã§...');
  ...
  const loadComments = () => {
    ...
    if (!currentPost) return;
    commentStatus.loading('?ìÍ???Î∂àÎü¨?§Îäî Ï§ëÏûÖ?àÎã§...');
    fetch(`${COMMUNITY_POST_BASE}/${encodeURIComponent(postId)}/comments`, { headers: { Accept: 'application/json' } })
      ...
      .catch(() => { commentStatus.error('?ìÍ???Î∂àÎü¨?§Ï? Î™ªÌñà?¥Ïöî.'); currentComments = []; renderComments(); });
  };
  ...
  const submitComment = (e) => {
    ...
    if (usingMock) {
      ...
      el.cmtInput.value = '';
      updateCommentCounts(currentComments.length); renderComments(); commentStatus.message('?òÌîå ?ìÍ????±Î°ù?òÏóà?µÎãà??'); setTimeout(() => commentStatus.clear(), 1200); el.cmtSubmit.disabled = false; return;
    }
    const au = getAuthUser();
    if (!au?.id) { commentStatus.error('∑Œ±◊¿Œ¿Ã « ø‰«’¥œ¥Ÿ.'); el.cmtSubmit.disabled = false; return; } const body = { postId, userId: au.id, content };
    ...
    fetch(`${COMMUNITY_COMMENT_BASE}`, { method: 'POST', headers, body: JSON.stringify(body) })
      ...
      .then(() => { el.cmtInput.value = ''; commentStatus.message('?ìÍ????±Î°ù?òÏóà?µÎãà??'); loadComments(); setTimeout(() => commentStatus.clear(), 1200); })
      .catch(() => { commentStatus.error('?ìÍ? ?±Î°ù Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.'); })
  ...
  const deletePost = () => {
    ...
    if (!confirm('??Í≤åÏãúÍ∏Ä????†ú?†Íπå??')) return;
    const btn = el.postDeleteButton; if (btn) btn.disabled = true;
    if (usingMock) { status.message('?òÌîå Í≤åÏãúÍ∏Ä????†ú?òÏóà?µÎãà??'); setTimeout(() => { window.location.href = 'community.html'; }, 800); if (btn) btn.disabled = false; return; }
    ...
    fetch(`${COMMUNITY_POST_BASE}/${encodeURIComponent(postId)}?userId=${encodeURIComponent(userId ?? '')}&admin=${String(admin)}`, { method: 'DELETE', headers: Object.assign({}, getAuthHeader()) })
      ...
      .then(() => { status.message('Í≤åÏãúÍ∏Ä????†ú?òÏóà?µÎãà??'); setTimeout(() => { window.location.href = 'community.html'; }, 800); })
      .catch((err) => { if (String(err && err.message) === 'forbidden') alert('Î≥∏Ïù∏ Í∏ÄÎß???†ú?????àÏñ¥??'); else alert('Í≤åÏãúÍ∏Ä ??†ú???§Ìå®?àÏäµ?àÎã§.'); })
  ...
  if (el.shareBtn) el.shareBtn.addEventListener('click', () => {
    ...
    if (navigator.clipboard?.writeText) { navigator.clipboard.writeText(shareData.url).then(() => alert('ÎßÅÌÅ¨Í∞Ä Î≥µÏÇ¨?òÏóà?µÎãà??')).catch(() => alert('ÎßÅÌÅ¨ Î≥µÏÇ¨???§Ìå®?àÏäµ?àÎã§.')); }
    else { prompt('ÎßÅÌÅ¨Î•?Î≥µÏÇ¨??Ï£ºÏÑ∏??, shareData.url); }
  });
